...
# Server prefix
serverPrefix = config.serverName + ":\n_ "
#set NEAR rpc url
...
config_server_name = config.serverName

lt_cpu = f"CPU {config_server_name}"
lt_ram = f"RAM {config_server_name}"
lt_disks = f"Disk usage {config_server_name}"
lt_linuxtools = f"Linux tools {config_server_name}"
lt_ping = f"Ping test {config_server_name}"
lt_traceroute = f"Traceroute test {config_server_name}"
lt_topproc = f"Top processes {config_server_name}"
lt_spdtst = f"Network speed test {config_server_name}"
lt_currntwrkload = f"Current network load {config_server_name}"
lt_currntdiskload = f"Current disk i/o {config_server_name}"
lt_starttime = f"Uptime {config_server_name}"
lt_mainmenu = f"Main menu {config_server_name}"
lt_docker_container = f"Docker Container {config_server_name}"
lt_nearpool = f"My pool info {config_server_name}"
lt_nearlogs = f"Near logs {config_server_name}"
lt_nearcurrent = f"Current {config_server_name}"
lt_nearproposals = f"Proposals {config_server_name}"
lt_nearnext = f"Next {config_server_name}"
...
def command_cpu(message):
  if message.from_user.id == config.tg:
    try:
      ...
      bot.send_message(config.tg, text=serverPrefix + cpu, parse_mode="Markdown")
...
def command_ram(message):
  if message.from_user.id == config.tg:
    try:
      ram = ("*RAM, Gb.*\n_Total: ") + str(subprocess.check_output(["free -mh | grep Mem | awk '{print $2}'"], shell = True,encoding='utf-8')) + ("Available: ") + str(subprocess.check_output(["free -mh |>
      swap = ("*SWAP, Gb.*\n_Total: ") + str(subprocess.check_output(["free -mh | grep Swap | awk '{print $2}'"], shell = True,encoding='utf-8')) + ("Available: ") + str(subprocess.check_output(["free -m>
      bot.send_message(config.tg, text=serverPrefix + server + ram + swap, parse_mode="Markdown")
...
def command_disk(message):
  if message.from_user.id == config.tg:
    try:
      ...
      bot.send_message(config.tg, text = serverPrefix + disk, parse_mode="Markdown", reply_markup=markup)
    except:
...
def command_linuxtools(message):
    try :
        if message.from_user.id == config.tg:
            ...
            containers = result_lines[1:]

            # Новый заголовок для вывода
            new_result = [f"{'ACTIVE':<7} {'NAMES':<30} {'STATUS':<30}"]

            for container in result_lines[1:]:  # Пропускаем заголовок
                parts = re.split(r'\s{2,}', container.strip())

                if len(parts) >= 5:
                    names = parts[-1]
                    status = parts[4]

                    # Определяем активность контейнера по STATUS
                    active = '🟢' if 'Up' in status else '🔴'

                    # Формируем новую строку с данными и статусом активности
                    new_result.append(f"{active:<7} {names:<30} {status:<30}")

            # Форматируем результат для отправки
            formatted_output = "\n".join(new_result)

            # Отправка форматированного текста
            bot.send_message(config.tg, text=f"```\n{formatted_output}\n```", parse_mode='MarkdownV2')
...
def command_pingcheck(message):
  if message.from_user.id == config.tg:
    try:
      ...
      bot.send_message(config.tg, text=serverPrefix + pingcheck, reply_markup=markuplinux)
...
def command_traceroutecheck(message):
  if message.from_user.id == config.tg:
    try:
      ...
      bot.send_message(config.tg, text=serverPrefix + traceroutecheck, reply_markup=markuplinux)
    except:
...
def command_timediff(message):
  if message.from_user.id == config.tg:
    try:
      ...
      bot.send_message(config.tg, text=serverPrefix + topps, reply_markup=markuplinux)
    except:
...
def command_srvstart(message):
  if message.from_user.id == config.tg:
    try:
      ...
      bot.send_message(config.tg, text=serverPrefix + startt, reply_markup=markuplinux)
    except:
...
#          i.write(str(int(time.time())) + ";" + netblockheight + localblockheight + syncdiff + "\n")
        if int(float(syncdiff)) >= config.syncalarm:
          if alrtprdsync in config.repeattimealarmnode:
            try:
                bot.send_message(config.tg,serverPrefix + "\U0001F6A8 " + ("Node out of sync ") + str(syncdiff) + (" blocks behind") )
            except:
              pass
            alrtprdsync +=5
          else:
      ...
      try:
        ...
        if int(float(blocksdiff)) >= config.blocksdiff:
          if alrtprdblocks in config.repeattimealarmnode:
            try:
                bot.send_message(config.tg, serverPrefix + "\U0001F6A8 " + str(blocksdiff) + ("blocks pruduced less than expected !!! "))
            except:
              pass
            alrtprdblocks +=30
          else:
      ...
      try:
        ...
        if int(float(chunksdiff)) >= config.chunksdiff:
          if alrtprdchunks in config.repeattimealarmnode:
            try:
                bot.send_message(config.tg,serverPrefix + "\U0001F6A8 " + str(chunksdiff) + ("chunks pruduced less than expected !!! "))
            except:
              pass
            alrtprdchunks +=30
          else:
      ...
      try:
        ...
        if int(float(cpuutilalert)) >= config.cpuutilalarm:
          if alrtprdcpu in config.repeattimealarmsrv:
            try:
              bot.send_message(config.tg,serverPrefix + "\U000026A1" + ("High CPU Utilization! ") + cpuutilalert + "%")
            except:
              pass
            alrtprdcpu +=5
          else:
      ...
      try:
        ...
        if int(float(pingc)) >= config.pingcalarm:
          if alrtprdpng in config.repeattimealarmsrv:
            try:
              bot.send_message(config.tg,serverPrefix + "\U000026A1 " + ("High ping! ") + pingc + " ms")
            except:
              pass
            alrtprdpng +=5
          else:
